...
void setActualGame(blocks_game::Game* game);
...
namespace blocks_game
{
  ...
  void Game::SetLastX(float value)
  {
    lastX = value;
  }
  ...
  void Game::SetLastY(float value)
  {
    lastY = value;
  }

  bool Game::IsFirstMouse() const
  {
    return firstMouse;
  }

  void Game::SetFirstMouse(bool value)
  {
    firstMouse = value;
  }
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
  if (actualGame->IsFirstMouse())
  {
    actualGame->SetLastX(xpos);
    actualGame->SetLastY(ypos);
    actualGame->SetFirstMouse(false);
  }

  float xoffset = xpos - actualGame->GetLastX();
  float yoffset = actualGame->GetLastY() - ypos;

  actualGame->SetLastX(xpos);
  actualGame->SetLastY(ypos);
  actualGame->GetCamera().ProcessMouseMovement(xoffset, yoffset);
}

void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
  actualGame->GetCamera().ProcessMouseScroll(yoffset);
}